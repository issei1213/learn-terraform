...
# RDS parameter group
# --------------------------------
resource "aws_db_parameter_group" "mysql_standalone_parameter-group" {
...
resource "aws_db_instance" "mysql_standalone" {
  ...
  db_name              = "tastlylog"
  parameter_group_name = aws_db_parameter_group.mysql_standalone_parameter-group.name
  ...
  // RDS削除時は、以下の設定でapplyを実行する。
  // その後、resource "aws_db_instance"をまるごとコメントアウトして、再度applyを実行する
